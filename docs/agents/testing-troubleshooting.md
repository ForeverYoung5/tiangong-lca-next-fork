...
lastReviewedNote: 'Reviewed the Issue #799 Dev regression follow-up: the focused serial Jest and Umi setup flow remains sufficient; no new recovery exception is needed.'
---
...
| `release:to-dev` returns `semantic_qualification_failed`, `semantic_qualification_scope_invalid`, or qualification provenance mismatch | the stale qualification could not complete, changed more than its exact receipt, or did not bind the clean source commit/tree | inspect the returned `.local/release-automation/**` log and receipt provenance; fix the first qualification failure and rerun the command, without hand-editing hashes or staging unrelated files |
| `release:to-dev` returns `release_preflight_failed` | the fully composed version/qualification/Docpact candidate is not production-ready | inspect the retained release log, fix the first locale/reference/qualification readiness failure in a separate scoped change, then rerun; no branch was pushed and no PR was created by the failed attempt |
...
| direct release E2E reports a stale qualification receipt, then refuses the dirty candidate after `e2e:qualify` succeeds | qualification correctly generated a new tracked receipt outside the deterministic version command, so the worktree is no longer a releasable immutable candidate | review the receipt, land it through a tracked `dev` PR, and retry from the clean merged candidate; for normal version delivery use `release:to-dev --apply`, which safely includes its generated receipt in the same Release PR |
