...
lastReviewedCommit: fd6362317111f234130af9c3fe3a85f46b469f34
lastReviewedNote: 'Reviewed for Platform #1086: the EdgeOne deploy bundle now carries an explicit version 3 route table generated by the pinned 1.4.6 CLI from the complete .edgeone/assets layout, with only its terminal fallback corrected to serve public/404.html with status 404, so unknown paths and missing scripts stop answering 200 with the application shell. Headers and the OAuth consent rewrite stay in the CLI-embedded conf block. Staging invalidates derived routing so a second build regenerates it for its own hashed assets; correction requires exactly one terminal file catch-all; staging refuses overlapping or symlinked paths. Evidence is offline only and the deployed status code remains a release-time measurement.'
---
