...
- Issue #1111 advances the exact installed `@tiangong-lca/tidas-sdk` dependency to `0.3.1`; the mapper-independent package contract additionally proves singleton and ordered non-empty Process review acceptance, while the coverage queue remains closed.
- Issue #1115 advances the exact installed `@tiangong-lca/tidas-sdk` dependency to `0.4.0`; the mapper-independent package contract additionally proves missing and default-empty Process general comments fail at the exact field path while localized content passes that field, and the coverage queue remains closed.
