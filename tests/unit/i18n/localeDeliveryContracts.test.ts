...
describe('shared locale delivery contracts', () => {
  ...
  it('covers every registry locale and mandatory route view without granting anonymous access', () => {
    ...
    expect(executableAssertions).toHaveLength(66);
    expect(APP_LOCALES).toEqual(SUPPORTED_APP_LOCALES);
    expect(new Set(executableAssertions.map(({ target }) => JSON.stringify(target))).size).toBe(66);
    ...
    expect(configuredPaths).toHaveLength(63);
    ...
    expect(evidenceSchema).toEqual(
      expect.objectContaining({
        ...
        properties: expect.objectContaining({
          ...
          assertions: expect.objectContaining({ minItems: 66, maxItems: 66 }),
          productionData: expect.objectContaining({
  ...
  it.each(SUPPORTED_APP_LOCALES)(
    ...
    (locale) => {
      ...
      expect(context.routeViewCoverage.derivedEvidence).toEqual(
        expect.objectContaining({
          configuredRouteCount: 63,
          coveredConfiguredRouteCount: 63,
      ...
      expect(executionEvidence?.assertionCount ?? 0).toBe(semanticRouteAndE2EReady ? 66 : 0);
